...
-(id)initWithTappedImageView:(UIImageView *)tappedImageView andImageUrls:(NSMutableArray *)imagesUrls andInitialIndex:(NSInteger )currentIndex dismissBlock:(void(^)() )block{
...
//    fromVCSnapShot = [self screenshot];
    ...
    if (self.fromImageView.image == nil || [self.fromImageView.image isEqual:[UIImage imageNamed:@"placeHolder"]]) {
        
        [self.fromImageView setImage:[UIImage imageNamed:@"placeHolder"]];
    }
    ...
    [UIView animateWithDuration:ANIMATEDURATION delay:0.0 usingSpringWithDamping:ANIMATEDAMPING initialSpringVelocity:0.0 options:UIViewAnimationOptionCurveEaseInOut animations:^{
        ...
        [self loadingImage:(UIImageView *)self.photosGalleryScroll.photos[self.initialPageIndex-1] withURL:_imagesUrls[self.initialPageIndex-1] shouldCompleted:YES];
    ...
    [self.photosGalleryScroll DidEndDecelerateBlock:^(NSInteger currentIndex) {
        ...
        [self blockToCallchangeImageView:currentIndex];

    }];
    
}
-(void)blockToCallchangeImageView:(NSInteger)index
{
...
-(void)loadingImage:(UIImageView *)needLoadingImageView withURL:(NSString *)url shouldCompleted:(BOOL)flag{
    ...
    if (!isInCache) {
        for (UIView *v in needLoadingImageView.subviews) {
    ...
    [needLoadingImageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:nil options:SDWebImageProgressiveDownload progress:^(NSInteger receivedSize, NSInteger expectedSize) {
        ...
        [self.fromImageView setImage:image];
...
-(void)dismissPhotoGalleryAnimated:(BOOL)animated{
    ...
    [self.animatedImageView setImage:self.fromImageView.image];
